...
# Set API key (optional — works without it via deterministic fallback)
...
5. **Compositor** generates TTS narration (edge-tts with xAI/OpenAI fallback), ambient music, and mixes audio into the final MP4
...
graph TB
    ...
    H --> I[TTS Narration<br/>edge-tts -> xAI -> OpenAI]
...
| `--tts-provider` | TTS provider: `edge`, `xai`, `openai`, or `none` |
...
When ffmpeg is available, the runner generates a sampled-frame contact sheet for quick visual QA:
...
**TTS provider chain (tried in order):**

| Priority | Provider | Notes |
|---|---|---|
| 1 | `edge` | Microsoft Edge TTS (free, no key required) |
| 2 | `xai` | xAI TTS (requires `XAI_API_KEY`) |
| 3 | `openai` | OpenAI TTS (requires `OPENAI_API_KEY`) |

The first provider that succeeds is used. `metadata.json` records `tts.actual_provider`. If all fail and narration text exists, the pipeline exits with an error.
...
| `contact_sheet.jpg` | Sampled-frame grid for taste QA (when ffmpeg available) |
