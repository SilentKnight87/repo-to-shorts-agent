...
**Goal:** Add a tiny local browser UI so a user can paste a GitHub URL or local path, click Generate, and open the generated short-video package without needing to remember CLI commands.
...
**Objective:** Let the operator launch the UI with one command.
...
**Objective:** Confirm the operator can open the UI from laptop when intentionally bound to LAN.
